...
class AIQueryService {
    ...
    async translateToAPICall(options) {
        try {
            if (!this.geminiConfig.apiKey || this.geminiConfig.apiKey.trim() === '') {
                return {
            ...
            const prompt = this.buildAPIPrompt(options);
            const response = await this.callGeminiAPIWithRetry(prompt);
...
Respond with ONLY the SQL query:
    ...
    buildAPIPrompt(options) {
        const { apiEndpoints, naturalQuery } = options;
        const endpointsDescription = apiEndpoints?.map(endpoint => `${endpoint.method} ${endpoint.path} - ${endpoint.description}`).join('\n') || 'No endpoints available';
        return `
...
Respond with ONLY the JSON object:
    ...
    async callGeminiAPIWithRetry(prompt) {
        let lastError;
    ...
    parseSQLResponse(response, connectionType) {
    ...
    calculateSQLConfidence(query) {
        let confidence = 0.5;
        const sqlKeywords = ['SELECT', 'FROM', 'WHERE', 'JOIN', 'GROUP BY', 'ORDER BY', 'HAVING'];
        const foundKeywords = sqlKeywords.filter(keyword => query.toUpperCase().includes(keyword));
        confidence += (foundKeywords.length / sqlKeywords.length) * 0.3;
        const dangerousKeywords = ['DROP', 'DELETE', 'UPDATE', 'INSERT', 'TRUNCATE', 'ALTER'];
        const hasDangerous = dangerousKeywords.some(keyword => query.toUpperCase().includes(keyword));
        if (hasDangerous) {
    ...
    calculateAPIConfidence(apiCall) {
        let confidence = 0.5;
